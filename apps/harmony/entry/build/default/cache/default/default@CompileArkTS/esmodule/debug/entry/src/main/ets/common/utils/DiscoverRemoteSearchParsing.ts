import { formatInstagramFeedTitle, formatXFeedTitle, normalizeSocialFeedDescription, } from "@bundle:com.livo.harmony/entry/ets/common/utils/SocialFeedTitles";
...
function extractAttribute(value: string, attribute: string): string {
    const matched = value.match(new RegExp(`\\b${attribute}=["']([^"']+)["']`, 'i'));
...
function resolveUrl(value: string, baseUrl: string): ResolvedProfileUrl | undefined {
    ...
    if (trimmed.startsWith('//')) {
        ...
        if (trimmed.startsWith('/')) {
            ...
            resolved = `${protocol}://${baseHost}${baseDir}${trimmed}`.replace(/([^:]\/)\/+/g, '$1');
        }
    }
...
function extractImageUrl(block: string): string {
    const imageMatch = block.match(/<img\b[^>]*\b(?:data-src|data-original|src)=["']([^"']+)["']/i);
    return imageMatch?.[1] ? decodeBasicEntities(imageMatch[1]) : '';
}
function shouldKeepProfile(query: string, username: string, title: string, block: string): boolean {
    const keyword = normalizeValue(query);
    if (!keyword) {
        return true;
    }
    return (normalizeValue(username).includes(keyword) ||
...
export function extractXFollowersFromText(text: string): string {
    ...
    const directMatch = normalized.match(/([\d]+(?:[.,]\d+)?(?:\s*[KMB万亿])?)\s*(?:followers?|位?粉丝)/i);
    if (directMatch?.[1]) {
        return `${directMatch[1].replace(/\s+/g, '').trim()} followers`;
    }
    const prefixedMatch = normalized.match(/(?:followers?|粉丝)\s*[:：]?\s*([\d]+(?:[.,]\d+)?(?:\s*[KMB万亿])?)/i);
    if (prefixedMatch?.[1]) {
...
function parseProfilesFromHtml(html: string, query: string, baseUrl: string, allowedHosts: string[], fallbackTitlePrefix: string): SocialRemoteProfile[] {
...
export function parseXProfilesFromSearchHtml(html: string, query: string): SocialRemoteProfile[] {
    return parseProfilesFromHtml(html, query, 'https://x.com', ['x.com', 'twitter.com'], '@');
}
export function parseInstagramProfilesFromSearchHtml(html: string, query: string): SocialRemoteProfile[] {
    return parseProfilesFromHtml(html, query, 'https://www.instagram.com', ['instagram.com'], '@');
}
function normalizeProfileTitle(profile: SocialRemoteProfile): string {
    return profile.title.trim() || profile.username;
}
function normalizeProfileUrl(platform: DiscoverRemoteProfilePlatform, username: string): string {
    if (platform === 'x') {
        return `https://x.com/${encodeURIComponent(username)}`;
    }
    return `https://www.instagram.com/${encodeURIComponent(username)}/`;
}
export function buildXProfileSeedFromProfile(profile: SocialRemoteProfile): DiscoverRemoteProfileSeed {
    return {
        platform: 'x',
...
export function buildInstagramProfileSeedFromProfile(profile: SocialRemoteProfile): DiscoverRemoteProfileSeed {
    return {
        platform: 'instagram',
        username: profile.username,
        title: normalizeProfileTitle(profile),
...
export function buildXCandidateFromProfile(profile: DiscoverRemoteProfileSeed, viewMapping: DiscoverRemoteViewMapping): DiscoverRemoteCandidateCompatible {
    return toDiscoverCandidateFromProfileSeed(profile, viewMapping);
}
export function buildInstagramCandidateFromProfile(profile: DiscoverRemoteProfileSeed, viewMapping: DiscoverRemoteViewMapping): DiscoverRemoteCandidateCompatible {
    return toDiscoverCandidateFromProfileSeed(profile, viewMapping);
}
export function dedupeAndLimitDiscoverCandidates(items: DiscoverRemoteProfileSeed[], limit: number): DiscoverRemoteProfileSeed[] {
...
export function toDiscoverCandidateFromProfileSeed(seed: DiscoverRemoteProfileSeed, viewMapping: DiscoverRemoteViewMapping): DiscoverRemoteCandidateCompatible {
    ...
    if (seed.platform === 'x') {
        return {
            ...
            description: normalizeSocialFeedDescription(seed.followers || 'X 用户', seed.profileUrl, seed.profileUrl),
    ...
    return {
        ...
        description: normalizeSocialFeedDescription(seed.followers || 'Instagram 用户', seed.profileUrl, seed.profileUrl),
        siteUrl: seed.profileUrl,
