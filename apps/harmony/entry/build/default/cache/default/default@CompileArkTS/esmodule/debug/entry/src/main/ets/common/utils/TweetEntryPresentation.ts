...
function extractText(summary: string, content: string): string {
    ...
    const paragraphMatches = Array.from(raw.matchAll(/<p\b[^>]*>([\s\S]*?)<\/p>/gi), (item: RegExpMatchArray) => normalizeParagraphWhitespace(stripHtml(item[1] || ''))).filter((line: string) => !!line && !isMetricsOnlyParagraph(line));
...
function extractUsername(source: TweetPresentationSource): string {
    const byUrl = extractUrlUsername(source.feedImageUrl) ||
...
function basePresentation(source: TweetPresentationSource): TweetEntryPresentation {
    ...
    return {
        ...
        mediaUrls,
        publishedLabel: trimValue(source.publishedLabel) || formatPublishedLabel(source.publishedAt),
...
function parseQuotedTweetFromHtml(rawHtml: string): ParsedQuote | undefined {
    ...
    const paragraphMatches = Array.from(quoteRaw.matchAll(/<p\b[^>]*>([\s\S]*?)<\/p>/gi), (item: RegExpMatchArray) => normalizeWhitespace(stripHtml(item[1] || ''))).filter((line: string) => !!line);
...
export function presentTweetEntryFromEntry(entry: {
    ...
    publishedLabel?: string;
    mediaUrls?: string[];
}, avatarUrl: string): TweetEntryPresentation {
