...
export function canonicalInstagramFeedUrl(rawUrl: string, siteUrl: string = ''): string {
    const trimmed = (rawUrl || '').trim();
    const username = extractInstagramUsername(trimmed) || extractInstagramUsername(siteUrl);
    if (!username) {
        return trimmed;
    }
...
export function formatInstagramFeedTitle(candidateTitle: string | undefined, usernameOrUrl: string): string {
    const fallback = extractInstagramUsername(usernameOrUrl) ||
    ...
    const hasMirrorMarkers = /public\s+posts|picnob|watch instagram stories|view latest posts/i.test(cleaned);
    ...
    const fromDumporTitle = cleaned.match(/^([a-zA-Z0-9._]{1,30})\s+(?:watch instagram stories|view latest posts)/i);
...
export function formatXFeedTitle(candidateTitle: string | undefined, usernameOrUrl: string): string {
    const fallback = extractXUsername(usernameOrUrl) ||
        trimTitle(usernameOrUrl).replace(/^@/, '');
...
export function normalizeSocialFeedTitle(candidateTitle: string | undefined, feedUrl: string, siteUrl: string = ''): string {
...
export function normalizeSocialFeedDescription(description: string | undefined, feedUrl: string, siteUrl: string = ''): string {
