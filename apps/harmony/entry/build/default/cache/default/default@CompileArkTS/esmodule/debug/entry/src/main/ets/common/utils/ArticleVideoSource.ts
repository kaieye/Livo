...
function isSupportedVideoPageUrl(url: string): boolean {
    return /(?:youtube\.com\/(?:watch\?(?:[^#\s]*&)?v=|embed\/|shorts\/)|youtu\.be\/|bilibili\.com\/video\/|b23\.tv\/)/i.test(url);
}
export function selectArticleVideoUrls(articleUrl: string, feedMediaUrls: string[], extractedVideoUrls: string[]): string[] {
    const dedupedFeedMediaUrls = dedupeUrls(feedMediaUrls);
    const normalizedFeedMediaUrls = dedupedFeedMediaUrls.filter((url: string) => isDirectVideoUrl(url));
