...
export function detectBilibiliPageLoggedIn(html: string, stateText: string, displayName: string): boolean {
    return (hasBilibiliLoginMarker(html) ||
        hasBilibiliLoginMarker(stateText) ||
        !!(displayName || '').trim());
}
function cookieValue(cookieString: string, key: string): string {
    const matched = (cookieString || '').match(new RegExp(`(?:^|;\\s*)${key}=([^;]+)`));
...
function isBilibiliUrl(url: string): boolean {
    return /^https?:\/\/(?:[^/]+\.)?bilibili\.com(?:\/|$)/i.test((url || '').trim());
}
export function detectBilibiliLogin(input: BilibiliLoginDetectionInput): BilibiliLoginDetectionResult {
    const cookieGroups = (input.cookieGroups || []).filter((value: string) => !!value.trim());
    const sessdata = bilibiliSessDataFromCookies(...cookieGroups);
    const hasSessionCookie = !!sessdata ||
        hasCookieInGroups('DedeUserID', ...cookieGroups) ||
        hasCookieInGroups('bili_jct', ...cookieGroups);
    const effectiveUrl = (input.currentUrl ||
        input.pageState?.currentUrl ||
        '').trim();
    const navLoggedIn = input.nav?.code === 0 && input.nav?.data?.isLogin === true;
    const displayName = (input.nav?.data?.uname ||
        input.pageState?.displayName ||
        '').trim();
    const pageLoggedIn = input.pageState?.loggedIn === true ||
