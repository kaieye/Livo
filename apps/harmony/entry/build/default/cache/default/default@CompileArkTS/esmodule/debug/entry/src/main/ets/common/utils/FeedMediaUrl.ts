...
function resolveAbsoluteUrl(baseUrl: string, rawUrl: string): string {
    ...
    const baseDir = parsedBase.pathname.includes('/')
        ...
        : '/';
    return `${parsedBase.protocol}//${parsedBase.host}${baseDir}${trimmed}`.replace(/([^:]\/)\/+/g, '$1');
}
...
function isSupportedVideoPageUrl(url: string): boolean {
    ...
    return /(?:youtube\.com\/(?:watch\?(?:[^#\s]*&)?v=|embed\/|shorts\/)|youtu\.be\/|bilibili\.com\/video\/|b23\.tv\/)/i.test(normalized);
}
...
export function isDirectVideoUrl(url: string): boolean {
    ...
    return /\.(mp4|m4v|mov|webm|m3u8|mpd|avi|mkv|mp3|m4a|aac|flac|wav)(\?|#|$)/i.test(normalized);
}
export function extractFeedMediaUrls(itemBlock: string, baseUrl: string): string[] {
    ...
    const pushUrl = (candidate: string, mimeType: string): void => {
        ...
        if (!isDirectImageMimeType(mimeType) &&
    ...
    mediaContentTags.forEach((tag: string) => {
        pushUrl(extractAttribute(tag, 'url'), extractAttribute(tag, 'type') || extractAttribute(tag, 'medium'));
    });
    const mediaThumbnailTags = itemBlock.match(/<media:thumbnail\b[^>]*\/?>/gi) ?? [];
    mediaThumbnailTags.forEach((tag: string) => {
        pushUrl(extractAttribute(tag, 'url'), 'image/thumbnail');
    });
    const atomEnclosureTags = itemBlock.match(/<link\b[^>]*rel=["']enclosure["'][^>]*\/?>/gi) ?? [];
    atomEnclosureTags.forEach((tag: string) => {
        pushUrl(extractAttribute(tag, 'href'), extractAttribute(tag, 'type'));
    });
    return results;
}
export function appendFeedMediaUrlsToContent(content: string, mediaUrls: string[]): string {
    const normalizedContent = content || '';
    const uniqueUrls = dedupeUrls(mediaUrls);
    if (uniqueUrls.length === 0) {
        return normalizedContent;
    }
    const missingUrls = uniqueUrls.filter((url: string) => !normalizedContent.includes(url));
