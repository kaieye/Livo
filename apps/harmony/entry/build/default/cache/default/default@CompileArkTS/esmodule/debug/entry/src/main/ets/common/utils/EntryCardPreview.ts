...
export function resolveEntryCardImageUrl(target: EntryCardPreviewTarget): string {
    ...
    const supportedVideoUrls = extractSupportedVideoUrls([
        ...
        target.articleUrl,
        ...(target.mediaUrls ?? []),
    ].join('\n'), baseUrl, target.articleUrl);
    const selectedVideoUrls = selectArticleVideoUrls(target.articleUrl, target.mediaUrls ?? [], supportedVideoUrls);
    const videoPreviewUrl = buildVideoPreviewUrl(selectedVideoUrls[0] ?? '');
    if (videoPreviewUrl) {
        return videoPreviewUrl;
    }
    const mediaImageUrl = (target.mediaUrls ?? []).find((url: string) => isImageUrl(url));
    if (mediaImageUrl) {
        return mediaImageUrl;
    }
    return (extractImageUrls(`${target.summary}\n${target.content}`, baseUrl)[0] ?? '');
}
