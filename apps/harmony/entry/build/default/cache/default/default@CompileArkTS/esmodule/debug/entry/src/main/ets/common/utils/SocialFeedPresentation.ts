import { extractInstagramUsername, extractXUsername, normalizeSocialFeedDescription, normalizeSocialFeedTitle, } from "@bundle:com.livo.harmony/entry/ets/common/utils/SocialFeedTitles";
...
function isGenericInstagramIcon(value: string): boolean {
    ...
    return (normalized.includes('instagram.com/static/images/ico') ||
...
function isGenericXIcon(value: string): boolean {
    ...
    return (normalized.endsWith('/twitter.3.ico') ||
...
function scoreStoredFeedTitle(value: string, feedUrl: string, siteUrl: string): number {
...
export function resolveSocialFeedDisplayTitle(title: string, feedUrl: string, siteUrl: string): string {
    return normalizeSocialFeedTitle(title, feedUrl, siteUrl);
}
export function resolveSocialFeedDisplayDescription(description: string, feedUrl: string, siteUrl: string): string {
    return normalizeSocialFeedDescription(description, feedUrl, siteUrl);
}
export function resolveSocialFeedDisplayImageUrl(imageUrl: string, feedUrl: string, siteUrl: string, title: string): string {
    ...
    const xUsername = extractXUsername(feedUrl) ||
...
export function resolvePreferredStoredFeedTitle(existingTitle: string, incomingTitle: string, feedUrl: string, siteUrl: string): string {
    ...
    if (normalizedExisting && normalizedIncoming && normalizedExisting !== normalizedIncoming) {
        ...
        if ((instagramUsername && (loweredIncoming === instagramUsername || loweredIncoming === `@${instagramUsername}`))
...
export function resolvePreferredStoredFeedImageUrl(existingImageUrl: string, incomingImageUrl: string): string {
