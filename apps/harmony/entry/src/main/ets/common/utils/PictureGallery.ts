...
function isCompletePreviewPayload(
  cachedPayload?: CachedPicturePreviewLike,
): boolean {
  if (!cachedPayload) {
    return false
  }
  ...
  return (
    Array.isArray(cachedPayload.entries) &&
...
export function extractEntryGalleryImageUrls(
  ...
  const mediaUrls = uniqueUrls(
    (target.mediaUrls ?? []).filter((url: string) => isImageUrl(url)),
  )
...
export function createPictureGalleryTiles(
  photoUrls: string[],
  maxVisibleCount: number = 9,
): PictureGalleryTile[] {
  const visibleUrls = uniqueUrls(photoUrls).slice(0, maxVisibleCount)
  const overflowCount = Math.max(
    0,
    uniqueUrls(photoUrls).length - visibleUrls.length,
  )
...
export function shouldUseCachedPicturePreview(
  isPicturesPreview: boolean,
  cachedPayload?: CachedPicturePreviewLike,
): boolean {
