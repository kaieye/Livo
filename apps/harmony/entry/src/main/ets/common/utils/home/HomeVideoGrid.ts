...
export function resolveHomeVideoCardSubtitle(
  target: HomeVideoCardMetaTarget,
): string {
...
export function resolveHomeVideoCardTokens(
  target: HomeVideoCardThemeTarget,
): HomeVideoCardTokens {
  return {
...
export function chunkHomeVideoEntries<T extends HomeVideoGridEntryLike>(
  entries: T[],
  columns: number = resolveHomeVideoGridColumns(),
): T[][] {
