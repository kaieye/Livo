...
test('index page primes active-mode load-more prefetch aggressively after initial and reload paths', () => {
  assert.match(
    source,
  ...
  assert.match(
    source,
    /private primeActiveModeLoadMorePrefetch\(\s*reason: string,\s*delayMs: number = Index\.ACTIVE_MODE_PREFETCH_DELAY_MS,/s,
  )
