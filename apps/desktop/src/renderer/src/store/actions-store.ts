  ...
  importRules: async (json) => {
    try {
      ...
      for (const rule of imported) {
        if (!rule.id || !rule.name || !Array.isArray(rule.conditions) || !Array.isArray(rule.actions)) {
          errors.push(`Invalid rule: ${rule.name || "unknown"}`)
          continue
        }
        // Assign new ID to avoid conflicts
        valid.push({ ...rule, id: crypto.randomUUID ? crypto.randomUUID() : Math.random().toString(36).slice(2) })
      }
