// Typewriter — 打字机逐字动画工具（移植自 Harmony 端 common/utils/Typewriter.ts）
//
// 设计要点：
...
export class Typewriter {
  ...
  start(fullText: string, callbacks: TypewriterCallbacks, fromText = ''): void {
    this.cancel()
  ...
  private computeChunkSize(pendingLength: number): number {
    ...
    return Math.max(MIN_CHARS_PER_TICK, Math.min(MAX_CHARS_PER_TICK, preferred))
  }
