...
export function getImageProxyFallbackUrls(
  ...
  const candidates = [
    canUseFoloProxy(originalUrl) ? createProxyUrl(FOLO_PROXY_BASE, originalUrl, options) : "",
...
export function getThumbnailUrl(originalUrl: string, size = 80): string {
  return getProxiedImageUrl(originalUrl, { width: size, height: size, quality: 80 })
}

/**
 * Get a gallery-quality proxy URL for social media photos.
 */
export function getGalleryImageUrl(originalUrl: string, maxWidth = 600): string {
