/**
 * Full theme palette, ported from Harmony's `ThemeService.ThemePalette`
 * (apps/harmony/.../common/services/ThemeService.ets) and adapted to the
 * desktop's existing Apple-style color scheme.
 *
 ...
 * driven entirely by this palette (mirroring Harmony's `resolvePalette`).
 *
 * Token format:
 ...
 * - Harmony-style semantic color tokens (`--color-bg-primary`,
 ...
 *   ported pages (ArticleDetail, FeedDetail, Subscriptions, Discover, …).
 *
...
export function paletteToCssVariables(
  ...
  return {
    ...
    // Harmony-style semantic color tokens (direct use, full colors).
