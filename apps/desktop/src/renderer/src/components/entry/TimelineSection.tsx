      ...
      {shouldUseVirtualTimeline ? (
        ...
        groupedEntries.map((group) => (
          <div key={group.labelKey + group.label}>
            ...
            {group.entries.map((entry) => renderCard(entry, timelineIndexById.get(entry.id) ?? 0))}
          </div>
        ))
      ) : (
        renderedEntries.map((entry) => renderCard(entry, timelineIndexById.get(entry.id) ?? 0))
      )}

      {!shouldUseVirtualTimeline && renderedEntries.length < timelineEntries.length && (
