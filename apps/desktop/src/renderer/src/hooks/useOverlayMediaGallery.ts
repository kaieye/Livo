...
export function useOverlayMediaGallery({
  ...
  const getPhotoToken = useCallback((photo?: { url?: string; previewUrl?: string }): string => {
    if (!photo) return ""
  ...
  const getPhotoInitialSrc = useCallback((photo?: { url?: string; previewUrl?: string }): string => {
    if (!photo) return ""
    if (photo.previewUrl) {
