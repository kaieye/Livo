...
function readMediaAttrs(raw: unknown): MediaAttrs | undefined {
  ...
  const attrs = (rec.$ && typeof rec.$ === "object" ? rec.$ : rec) as Record<string, unknown>
...
function pickImgUrlFromTag(tagHtml: string): string | null {
  ...
  const bestSrcset = pickBestFromSrcset(srcset) || pickBestFromSrcset(dataSrcset)
...
function collectMediaThumbnailNodes(item: Record<string, unknown>): unknown[] {
  ...
  if (groups) {
    ...
    for (const group of groupList) {
      ...
      if (groupThumb) nodes.push(...(Array.isArray(groupThumb) ? groupThumb : [groupThumb]))
    }
  }
...
function isKnownSocialMirrorImageUrl(url: string): boolean {
  ...
  return (
    /^https?:\/\/media\.(?:picnob|pixnoy|piokok|pixwox)\.[^/]+\//i.test(value) ||
...
export function normalizeKnownMediaUrl(rawUrl: string): string {
  ...
  try {
    ...
    if (isMirrorHost) {
      ...
      const rawQuery = questionIndex >= 0 ? decoded.slice(questionIndex + 1) : ""
      const markerIndex = rawQuery.indexOf("url=")
      let rawSlice = markerIndex >= 0 ? rawQuery.slice(markerIndex + 4).trim() : ""
      if (rawSlice) {
        try {
      ...
      if (encodedOrigin) {
        ...
        const nestedUrl = decodedOrigin.match(/https?:\/\/\S+/i)?.[0] || decodedOrigin
 ...
 */
export function extractMedia(item: Record<string, unknown>): MediaItem[] | undefined {
  ...
  if (enclosure?.url) {
    ...
    const urlLooksLikeImage = /\.(jpg|jpeg|png|webp|gif|bmp|avif)(\?|$)/i.test(enclosureUrl) ||
    ...
    if (type.startsWith("audio/")) {
    ...
    } else if ((type.startsWith("image/") || urlLooksLikeImage) && !isDecorativeInstagramAssetUrl(enclosure.url)) {
      ...
      const mirrorPreview = (finalUrl !== decodeHTMLEntities(enclosure.url) && isMirrorProxyUrl(enclosure.url))
        ? decodeHTMLEntities(enclosure.url) : undefined
  ...
  if (contentItems.length > 0) {
    for (const mc of contentItems) {
      ...
      const normalizedMediaUrl = mediaUrl ? normalizeKnownMediaUrl(mediaUrl) : ""
      ...
      if (mediaUrl && !media.some((m) => m.url === dedupeUrl)) {
        ...
        const urlLooksLikeImage = /\.(jpg|jpeg|png|webp|gif|bmp|avif)(\?|$)/i.test(decodedUrl) ||
        ...
        // keep the original mirror URL as previewUrl fallback — CDN signed URLs expire.
        const mirrorPreview = (itemType === "photo" && normalizedMediaUrl && normalizedMediaUrl !== decodeHTMLEntities(mediaUrl || "") && isMirrorProxyUrl(mediaUrl || ""))
          ? decodeHTMLEntities(mediaUrl || "") : undefined
        media.push({
  ...
  const content = unwrapXmlValue(item["content:encoded"]) || unwrapXmlValue(item.content) || unwrapXmlValue(item["description"]) || unwrapXmlValue(item.summary) || ""
  ...
  while ((match = imgRegex.exec(content)) !== null) {
    ...
    if (url && !media.some((m) => m.url === url) && !isTrackingPixel(url) && !isDecorativeInstagramAssetUrl(url)) {
      // Try to extract width/height from attributes
      const wMatch = match[0].match(/width=["']?(\d+)/)
      const hMatch = match[0].match(/height=["']?(\d+)/)
      const mirrorPreview = (url !== decodeHTMLEntities(rawUrl) && isMirrorProxyUrl(rawUrl))
        ? decodeHTMLEntities(rawUrl) : undefined
      media.push({
        url,
        type: "photo",
  ...
  while ((match = lazyImgRegex.exec(content)) !== null) {
    ...
    if (url && !media.some((m) => m.url === url) && !isTrackingPixel(url) && !isDecorativeInstagramAssetUrl(url)) {
      const mirrorPreview = (url !== decodeHTMLEntities(imgRawUrl) && isMirrorProxyUrl(imgRawUrl))
        ? decodeHTMLEntities(imgRawUrl) : undefined
  ...
  while ((match = anchorRegex.exec(content)) !== null) {
    ...
    if (href && isLikelyImageUrl(href) && !media.some((m) => m.url === href) && !isDecorativeInstagramAssetUrl(href)) {
      const mirrorPreview = (href !== decodeHTMLEntities(match[1]) && isMirrorProxyUrl(match[1]))
        ? decodeHTMLEntities(match[1]) : undefined
  ...
  while ((match = videoSrcRegex.exec(content)) !== null) {
    ...
    const urlLooksLikeImage = /\.(jpg|jpeg|png|webp|gif|bmp|avif)(\?|$)/i.test(urlLower) ||
  ...
  while ((match = videoPosterRegex.exec(content)) !== null) {
    ...
    if (videoSrc && posterUrl) {
      ...
      const videoItem = media.find((m) => m.url === decodedVideoSrc && m.type === "video")
      if (videoItem) videoItem.previewUrl = normalizeKnownMediaUrl(posterUrl) || posterUrl
      else {
        // If video not found with decoded URL, try all videos (fallback)
        const lastVideo = [...media].reverse().find((m) => m.type === "video")
        if (lastVideo && !lastVideo.previewUrl) lastVideo.previewUrl = normalizeKnownMediaUrl(posterUrl) || posterUrl
      }
    }
  }

  // 5. Extract YouTube videos from content
  const youtubeRegex = /(?:https?:\/\/)?(?:www\.)?(?:youtube\.com\/(?:watch\?v=|embed\/|shorts\/)|youtu\.be\/)([a-zA-Z0-9_-]+)/g
  ...
  const bilibiliRegex = /(?:https?:\/\/)?(?:www\.)?bilibili\.com\/video\/(BV[a-zA-Z0-9]+)|(av\d+)/g
  ...
  if (itunesData?.image && !media.some((m) => m.url === itunesData!.image)) {
    if (!media.some((m) => m.type === "photo")) {
      media.push({ url: normalizeKnownMediaUrl(itunesData.image) || itunesData.image, type: "photo" })
    }
  }
  ...
  for (const thumbNode of thumbnailItems) {
    ...
    if (!thumbUrl || media.some((m) => m.url === (normalizedThumbUrl || decodeHTMLEntities(thumbUrl)))) continue
    media.push({
  ...
  if (durationSeconds && durationSeconds > 0) {
    ...
    const target = media.find((m) => (m.type === "video" || m.type === "audio") && !m.duration)
...
export function deriveImageUrl(item: Record<string, unknown>): string {
  ...
  for (const mc of mediaContentItems) {
    const attrs = readMediaAttrs(mc)
    if (attrs?.url && (attrs.type?.startsWith("image/") || attrs.medium === "image")) {
  ...
  if (itunes?.image) {
    if (!isDecorativeInstagramAssetUrl(itunes.image)) return normalizeKnownMediaUrl(itunes.image) || itunes.image
  }

  const thumbnailItems = collectMediaThumbnailNodes(item)
  for (const thumbNode of thumbnailItems) {
    const thumbAttrs = readMediaAttrs(thumbNode)
    if (thumbAttrs?.url) return normalizeKnownMediaUrl(thumbAttrs.url) || thumbAttrs.url
  }
  ...
  if (content) {
    ...
    for (const tag of imgTags) {
      ...
      if (normalizedTagUrl && !isTrackingPixel(normalizedTagUrl) && !isDecorativeInstagramAssetUrl(normalizedTagUrl)) return normalizedTagUrl
    }
    ...
    for (const hrefMatch of hrefMatches) {
      ...
      if (decodedHref && isLikelyImageUrl(decodedHref) && !isDecorativeInstagramAssetUrl(decodedHref)) return decodedHref
    }
  }
...
export function extractAuthorAvatar(
  ...
  const author = item.author as { avatar?: string; image?: string; uri?: string } | string | undefined
