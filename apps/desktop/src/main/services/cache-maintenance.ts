...
export async function runCacheMaintenance(settings: AppSettings): Promise<void> {
  if (!settings.data.autoCleanCache) return

  const cacheLimitBytes = Math.max(0, settings.data.cacheSizeLimitMB || 0) * 1024 * 1024
