...
const PROVIDER_CONFIGS: Record<AccountProvider, ProviderConfig> = {
  youtube: {
    loginUrl: "https://accounts.google.com/ServiceLogin?continue=https://m.youtube.com/",
    title: "Sign in to YouTube",
    cookieDomains: [".youtube.com", ".google.com", ".accounts.google.com", "youtube.com", "accounts.google.com"],
...
const accountNameFile = join(app.getPath("userData"), "account-display-names.json")
...
function saveAccountNames(data: Partial<Record<AccountProvider, string>>): void {
  try {
...
async function fetchBilibiliNavState(): Promise<{ loggedIn: boolean; displayName: string | null }> {
  try {
...
async function tryResolveDisplayName(provider: AccountProvider): Promise<string | null> {
  ...
  if (provider === "instagram") {
    const html = await fetchTextWithSession("https://www.instagram.com/accounts/edit/")
    if (!html) return null
  ...
  if (provider === "youtube") {
    ...
    const google2 = await fetchTextWithSession("https://myaccount.google.com/personal-info")
    if (google2) {
...
async function hasProviderAuthCookie(provider: AccountProvider): Promise<boolean> {
  const config = PROVIDER_CONFIGS[provider]
  try {
    const allCookies = await session.defaultSession.cookies.get({})
    return allCookies.some((cookie) => {
      const domain = (cookie.domain || "").replace(/^\./, "").toLowerCase()
      const domainMatched = config.cookieDomainKeywords.some((kw) => domain.endsWith(kw))
...
export async function getAccountState(provider: AccountProvider): Promise<AccountSessionState> {
  if (provider === "bilibili") {
    const nav = await fetchBilibiliNavState()
    if (!nav.loggedIn) {
      return {
        ...
        linked: false,
        displayName: null,
        error: "Bilibili login session not detected. Please relink your account.",
      }
    }
    return {
      provider,
  ...
  try {
    ...
    for (const domain of config.cookieDomains) {
      ...
      const hasAuth = cookies.some((c) => config.authCookieNames.includes(c.name))
      if (hasAuth || hasCookieByGlobalScan) {
        ...
        if (!displayName) {
          return {
            ...
            linked: false,
            displayName: null,
            error: "Found login session cookies, but failed to resolve account name. Please set it manually.",
          }
        }
        return {
          provider,
...
export async function linkAccount(provider: AccountProvider): Promise<{ success: boolean; error?: string }> {
  const config = PROVIDER_CONFIGS[provider]
  ...
  return new Promise((resolve) => {
    ...
    try {
      ...
      finish({ success: false, error: `Failed to create login window: ${String(err)}` })
      return
    }

    try {
      loginWin.webContents.setUserAgent(config.userAgent === "desktop" ? DESKTOP_UA : MOBILE_UA)
    } catch {
      // ignore
    }
    ...
    loginWin.webContents.on("render-process-gone", () => {
      finish({ success: false, error: "Login window renderer crashed. Please retry." })
    ...
    loginWin.webContents.on("did-fail-load", (_event, errorCode, _errorDescription, validatedURL) => {
    ...
    const cookiePoll = setInterval(async () => {
      if (settled) return
      const hasAuth = provider === "bilibili"
    ...
    loginWin.loadURL(config.loginUrl).catch((err) => {
      failedToOpen = true
      finish({ success: false, error: `Failed to open login page: ${String(err)}` })
      if (!loginWin.isDestroyed()) loginWin.close()
    })
    ...
    loginWin.on("closed", async () => {
      ...
      if (!hadNavigation) {
        finish({ success: false, error: "Login window did not navigate successfully; check your network and retry" })
        return
      }
      finish({ success: false, error: "No login session detected. Complete login before closing the window." })
    })
  })
}

export async function unlinkAccount(provider: AccountProvider): Promise<{ success: boolean; error?: string }> {
  const config = PROVIDER_CONFIGS[provider]
  try {
    for (const domain of config.cookieDomains) {
      const cookies = await session.defaultSession.cookies.get({ domain })
      for (const cookie of cookies) {
        const cookieDomain = cookie.domain?.replace(/^\./, "") || domain.replace(/^\./, "")
