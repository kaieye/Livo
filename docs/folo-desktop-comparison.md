...
短板在更完整的发布治理：已有基础质量门禁、Node/pnpm 约束、Dependabot、release plan 和 changelog 草稿，但还没有 runtime version、channel 管理、CI artifact 聚合、签名/公证和 release attestation。

### 建议

- P1：补 workspace 级依赖治理策略，明确 overrides、patchedDependencies 和依赖升级规则。
- P1：建立 `release.json` 或等价版本元数据，以及版本 bump 流程。
...
Folo 的 deep link 是实际产品入口。它同时注册 legacy/new 协议，支持 dev 模式注册，收到 URL 后能分发到添加订阅、discover、feed、list、refresh、auth 等动作。Livo 当前 `livo://` 只是保存为 `pendingProtocolUrl`，窗口 ready 后记录日志并清空，没有真正进入路由或订阅流程。

Folo 的窗口状态管理更完整。窗口位置和大小会持久化，恢复时 clamp 到当前屏幕工作区，避免多屏变化后窗口跑到屏幕外；macOS 有 vibrancy、traffic light、退出全屏后刷新 bounds 的修复。Livo 当前固定初始尺寸，没有 bounds 恢复。
...
Folo 的 entry column 拆分更健康。条目列表、网格、item template、hooks、滚动已读、刷新恢复、虚拟化等拆在 `modules/entry-column`。Livo 的 `EntryList.tsx` 约 2081 行，社会流、图片网格、虚拟列表、AI 状态、上下文菜单、媒体判断、加载策略交织在一起。

Folo 的正文渲染接口更深。正文布局用 `ArticleLayout`、`MediaLayout`、`SocialMediaLayout` factory；HTML/Markdown 渲染、ShadowDOM、TOC、readability notice、附件区、转录切换等模块化。Livo 的 `EntryContent.tsx` 约 1600 行，阅读进度、readability、AI 摘要、翻译、媒体、外链警告、正文渲染和音频面板都集中在一个实现里。
...
- P0：先拆 `EntryList.tsx`，按 `entry-column/list/grid/items/hooks` 拆，不改变用户行为。
- P0：再拆 `EntryContent.tsx`，按 `entry-content/layouts/renderers/accessories/hooks` 拆。
...
Livo 有 Quick Search，feed 搜索主要是 includes，entry 搜索是 SQL `LIKE`，Web 端会 `getAll()` 后内存过滤。基础可用，但排序、模糊匹配、订阅维度、列表/收件箱维度都弱。
...
- P1：引入本地 Fuse search index，覆盖 feed、entry、subscription。
- P1：Quick Search 支持类型过滤和 ranking。
...
Folo 有 GitHub Actions 质量门禁：PR 和 push 会安装依赖、构建 Web/SSR，再跑 format、typecheck、lint、test。Livo 当前没有 `.github` CI。

Folo 有 Playwright E2E，分 web 和 Electron 两个 project，覆盖注册、登录、订阅 onboarding feed、退订、timeline 切换、打开文章、读/未读、双浏览器设置同步；失败保留 trace、截图和视频。Livo 当前没有 E2E harness。
...
- P0：补 Playwright Web/Electron E2E：启动、添加订阅、刷新、打开文章、读/未读、设置持久化。
...
Folo 有长期 changelog，`apps/desktop/changelog` 从 0.1.x 到 1.9.0 都有版本记录，还有 `next.md` 和 template。Livo 已补 `docs/changelog/next.md` 和 `docs/release/release-plan.md`，但还没有历史版本目录和正式发布模板。
...
1. Playwright Web/Electron E2E 核心路径。
2. `livo://` deep link 分发。
3. 拆 `EntryList.tsx` 和 `EntryContent.tsx`，先做不改行为的模块深度提升。
4. 独立 `subscriptions` 模型，拆开 Feed 元数据和订阅关系。
5. MCP 动态工具接入层，复用现有 Agent 工具注册表。
...
4. 用户可配置 AI Task。
5. release metadata、历史 changelog、签名/公证预留配置。
6. 可关闭的异常上报 adapter。
...
3. 把 `EntryList.tsx` 拆为 `entry-column` 模块。
4. 把 `EntryContent.tsx` 拆为 `entry-content` 模块，并抽 `getEntryContentLayout`。
