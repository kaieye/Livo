...
| API/IPC 契约 | Zod 校验 + `ok/fail` 统一响应 + `AppError` | 已建立 IPC channel type guard、统一 Envelope、`registerChannel` 包装和 Web adapter API 形状测试 | 继续补 handler 级契约测试 |
| 测试 | 约 218 个测试文件，覆盖 repository/service/route/worker/UI utils | 约 62 个测试文件，集中在纯工具与部分 service | 优先补 handler、store、SQLite adapter、列表模型、任务运行器测试 |
| 安全 | SSRF guard、media proxy guard、结构化错误 | 外链 sanitizer、Electron proxy、session header policy | 增加 URL 安全策略 Module，按桌面场景调整规则 |
| 文档/部署 | deploy/development 清晰区分 | README 已说明 SQLite 和旧 JSON 自动迁移 | 补充开发入口和架构文档，避免事实漂移 |
...
- 缺口集中在 IPC handlers、store action、SQLite adapter 集成、复杂 UI 派生逻辑。
...
- `docs/deploy.md`
- `.env.example`
- `Dockerfile` / `deploy/compose.yaml`

FeedFuse 把“开发”和“部署”区分得很明确。

**Livo 当前状态**

- README 已说明桌面端核心数据保存在 SQLite，并支持从早期 JSON 格式自动迁移。
- README 没有清楚解释桌面端与 Web 入口能力差异。

**建议**

- 增加 `docs/development.md`：桌面开发、Web 入口限制、Electron 镜像、常用验证命令。
- 增加 `docs/architecture.md`：main/preload/renderer/shared/web 分层、IPC 契约、数据路径。
...
2. shared types 拆分。
3. Web 入口能力矩阵文档。
4. 将视频时长、Fever sync 纳入 Task Runner。
