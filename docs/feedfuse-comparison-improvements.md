...
| 异步任务     | pg-boss 队列，`QUEUE_CONTRACTS` 集中配置重试/并发/去重           | 刷新、视频时长补全、订阅 bootstrap 已接入本地 Task Runner；AI summary/translation 状态仍不完全统一                         | 继续把长任务迁入可观察的 run 状态，不引入服务端队列    |
...
| 测试         | 约 218 个测试文件，覆盖 repository/service/route/worker/UI utils | 约 79 个测试文件，集中在纯工具与部分 service，宽视图条目派生逻辑已补纯函数契约测试                                         | 持续围绕关键契约补测试                                 |
| 安全         | SSRF guard、media proxy guard、结构化错误                        | 已建立共享 URL policy、外链 sanitizer、Electron proxy、session header policy                                               | 继续按桌面场景维护 URL 规则                            |
...
- 批量刷新、视频时长补全、添加社交订阅后的后台 bootstrap 已通过 Task Runner 记录 run 状态；事件只保留为实时提示。
- AI chat 有 stream event，AI summary/translation 和 digest 状态不完全统一。

**建议**

后续只继续处理 AI summary/translation 等仍未接入 run store 的长任务。UI 监听事件可以保留，但事件只做实时提示，最终状态以 run store 为准。
...
- `src/main/services` 已按 account/actions/ai/feed/fever/system/video 等目录收敛。
- `src/renderer/src/components/entry` 仍聚集大量不同视图。

**建议**

后续只在修改对应 entry 视图时顺手拆分，避免一次性搬家造成大面积 import churn。
