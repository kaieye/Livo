...
- [x] **1.6** 新增 AccountLogin 页面（账号登录流程） → 依赖: 0.1, 0.2 ✅
  - 实现:`pages/AccountLoginPage.tsx`(~140 行) — 顶部返回头 + “在设置中管理”跳转按钮、provider 列表（YouTube / X / Instagram / Bilibili），点击行触发 `window.api.accounts.link(provider)`（main 进程已注册的 WebView/Cookie 流程不动），关联成功后 `navigate(-1)` 回到调用方。每行的 `ProviderRow` 复用 `useAccountStatusQuery`，已关联状态实时显示
  - 路由:`/login/:provider?`(理由:`provider` 可选——`/login` 进入展示列表；`/login/:provider` 用于深链直接定位某个 provider 行并自动触发关联流。命名与 IPC `account.*` provider 字段对齐；非 `/account/:id` 那种需独立 account-id 系统的形状) — `route-paths.ts` 新增 `ROUTES.login()` + 扩 `NON_VIEW_PATHS`，`routes.tsx` 注册懒加载页
  - 文件:`pages/AccountLoginPage.tsx`(新建)、`components/account/provider-config.ts`(新建，`PROVIDER_CONFIGS` — 4 个 provider 的视觉/描述元数据，从 `AccountsSettings` `ACCOUNT_CARDS` 抽出 single source of truth)、`components/account/useProviderLink.ts`(新建，封装 `accounts.link()` 调用；返回 `status: 'idle' | 'loading' | 'success' | 'error'` 判别枚举，避免 locale-fragile 字符串嗅探)、`components/account/ProviderRow.tsx`(新建，列表型 provider 行视觉；通过 `useTranslation` 本地化 connected / signedInAs / linkSuccess / linkFailed 文案)、`lib/account-status.ts`(新增 `refreshAccountStatus(provider, queryClient)` 共享 util — AccountsSettings 与 useProviderLink 收敛到同一 status refresh 策略，消除 17 行 verbatim 重复)、`locales/{en,zh-CN}/account-login.ts`(新建 namespace)、`router/{routes,route-paths}.tsx`、`components/settings/AccountsSettings.tsx`(顶部新增"去登录页"按钮跳 `/login`；`ACCOUNT_CARDS` 改为复用 `PROVIDER_CONFIGS`；`refreshStatus` 改用共享 util)
  - 复用:`window.api.accounts.{status,link}`(main `account-handlers.ts` IPC 未改)、`useAccountStatusQuery`、`accountStatusQueryOptions`、`useSettingsStore.setOpen`(从设置页关闭 dialog 后导航)
  - 不在范围(留给 4.x):AccountsSettings 1044 行整体重写（4.x 设置系统完善）、取消关联流程移入新页（仍留在 settings 管理区）、Bilibili 关注预览/导入 UI（仅由设置页消费，未抽出）、account-data export（设置页 data 标签）、AccountLoginHandlerFactory 抽象（desktop main 已按 provider 分 handler，未出现重复无需工厂）、`AccountCard.handleLink` 改用 `useProviderLink`（entangled with manual display name / unlink / Bilibili followings 共享 loading state，整体迁移留 4.x；本轮已抽 status refresh 共享 util 完成 deferred refactor 的简单一半）
  - 遗留:Sidebar 仍把账号按钮指向 SettingsDialog（保守，避免砍掉旧入口）— 真正大改入口位置等 4.x 设置重构；i18n `account-login` namespace 已覆盖 page chrome + ProviderRow / useProviderLink 反馈，provider 描述仍中文硬编码在 `provider-config.ts`（与 AccountsSettings 现状一致，下次 i18n 批量过一遍）；`useProviderLink` 未做 unmount cancel（页面 unmount 期 IPC 仍 in-flight 可能后写 state，目前无观测到的问题，可在下个 hook 重构同时补 `useAbortController`）
