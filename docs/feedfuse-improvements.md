...
| --- | --- | --- | --- | --- |
| **P0 高价值（明确短板）** | | | | |
| 3 | AI 过滤规则接入 + AI 解读(Digest) 视图 | 已有语义过滤服务与 Digest 工具，缺 UI/调度/持久化 | 高 | 中-大 |
...
**Livo 现状**：已有主进程语义过滤判定服务，以及 Digest 候选 id 校验、分批和上下文预算工具；仍缺规则 UI/执行链接入、Digest 视图、运行记录和持久化。
...
**借鉴建议**：
1. **AI 过滤接入规则系统**：在自动化规则中增加语义条件的配置、预览和刷新链路接入，注意失败时不能误删文章。
2. **AI 解读视图**：新增"今日简报 / 本周趋势"视图，把现有 Digest 工具接到候选查询、生成、展示和缓存流程。
...
2. **#3（AI 过滤规则接入 / Digest 视图）** —— 在 #4 之后做，能复用提示词层和数据层。
3. **#9** 按产品方向择机。
