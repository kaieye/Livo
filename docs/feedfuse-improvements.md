...
| --- | --- | --- | --- | --- |
| **P0 高价值（明确短板）** | | | | |
| 3 | AI 解读(Digest) 视图 | 已有 Digest 工具，缺视图、调度和持久化 | 高 | 中-大 |
...
**Livo 现状**：已有 Digest 候选 id 校验、分批和上下文预算工具；仍缺 Digest 视图、运行记录和持久化。
...
**借鉴建议**：
1. **AI 解读视图**：新增"今日简报 / 本周趋势"视图，把现有 Digest 工具接到候选查询、生成、展示和缓存流程。
2. Livo 已有 Agent 工具系统，AI 解读可以复用其 OpenAI 调用与 trace 基础设施。
...
2. **#3（Digest 视图）** —— 在 #4 之后做，能复用提示词层和数据层。
3. **#9** 按产品方向择机。
