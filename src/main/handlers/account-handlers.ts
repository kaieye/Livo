...
import { getAccountState, linkAccount, setManualAccountDisplayName, unlinkAccount } from "../services/account-auth"
import { getBilibiliFollowings } from "../services/bilibili-followings"

const SUPPORTED_PROVIDERS: AccountProvider[] = ["youtube", "x", "instagram", "bilibili"]
...
export function registerAccountHandlers(): void {
  ipcMain.handle(IPC.ACCOUNT_STATUS, async (_event, provider: string) => {
    if (!isSupportedProvider(provider)) {
      return { provider, linked: false, displayName: null, error: "Unsupported provider" }
    }
  ...
  ipcMain.handle(IPC.ACCOUNT_SET_DISPLAY_NAME, async (_event, provider: string, displayName: string) => {
    if (!isSupportedProvider(provider)) {
      return { success: false, error: "Unsupported provider" }
    }
