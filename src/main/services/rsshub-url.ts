...
export function normalizeRsshubProtocolUrl(rawUrl: string, rsshubInstance: string): string {
  const trimmed = rawUrl.trim()
...
export function ensureInstagramUserFeedLimit(rawUrl: string, limit = 100): string {
  const trimmed = rawUrl.trim()
  if (!trimmed) return trimmed

  const normalizedLimit = Math.max(1, Number.isFinite(limit) ? Math.floor(limit) : 100)
