export function formatFeedTitle(
  feedUrl: string,
  parsedTitle?: string,
...
function extractTwitterNameFromTitle(title: string): string {
  ...
  const handleMatch = cleaned.match(
    /^(.+?)\s*\(\s*@([a-zA-Z0-9_]{1,15})\s*\)\s*(?:\/|[-\u2013\u2014]|on)\s*(?:x|twitter)\s*$/i,
  )
...
function extractTwitterUsernameFromFeedUrl(feedUrl: string): string {
  try {
    const u = new URL(feedUrl)
    const rsshubMatch = getPathLike(feedUrl).match(
      /\/twitter\/user\/([^/?#]+)/i,
    )
    if (rsshubMatch?.[1])
...
function extractInstagramNameFromTitle(title: string): string {
  ...
  const fromPicnobLead = cleaned.match(
