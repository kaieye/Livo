...
function getFeedKey(feed: Feed, normalizedUrl: string): string {
  ...
  const instagramUser = raw.match(/\/(?:instagram|picnob(?:\.info)?|pixnoy|piokok)\/user\/([^/?#]+)/i)?.[1]
  if (instagramUser) return `instagram:user:${decodeURIComponent(instagramUser).replace(/^@/, "")}`

  const twitterUser = raw.match(/\/(?:twitter|x)\/user\/([^/?#]+)/i)?.[1]
  if (twitterUser) return `twitter:user:${decodeURIComponent(twitterUser).replace(/^@/, "").toLowerCase()}`

  const bilibiliDynamic = raw.match(/\/bilibili\/user\/dynamic\/([^/?#]+)/i)?.[1]
  if (bilibiliDynamic) return `bilibili:dynamic:${decodeURIComponent(bilibiliDynamic)}`
...
function getNormalizedFeedUrl(feed: Feed): string {
  ...
  const canonicalFeedUrl = canonicalizeInstagramFeedUrl(feed.upstreamUrl || feed.url)
  const limitedUrl = ensureTwitterUserFeedLimit(ensureInstagramUserFeedLimit(canonicalFeedUrl, 100), 120)
...
function getDesiredSource(feed: Feed): AggregatedFeedPayload["source"] {
  ...
  if (explicit === "direct" || explicit === "local-agent" || explicit === "private-aggregator") {
    return explicit
  }
...
async function fetchDirectPayload(feed: Feed, options?: { force?: boolean }): Promise<AggregatedFeedPayload> {
  ...
  if (!result.data) throw new Error(`Direct provider returned empty data for ${normalizedUrl}`)
...
async function fetchLocalAgentPayload(feed: Feed, options?: { force?: boolean }): Promise<AggregatedFeedPayload> {
  const normalizedUrl = getNormalizedFeedUrl(feed)
  const key = getFeedKey(feed, normalizedUrl)
  const settings = getSettings().aggregator
  pruneAggregatorSnapshots(Math.max(1, settings.cacheRetentionDays) * 24 * 60 * 60 * 1000)

  const snapshot = getAggregatorSnapshot(key)
  const now = Date.now()
  const pollIntervalMs = Math.max(60, settings.pollIntervalSeconds || 900) * 1000
  ...
  try {
    ...
    if (result.notModified) {
      ...
      throw new Error(`Local agent got 304 before snapshot was initialized for ${normalizedUrl}`)
    }

    if (!result.data) throw new Error(`Local agent fetch returned empty data for ${normalizedUrl}`)
    setAggregatorSnapshot({
      key,
      source: "local-agent",
      fetchedAt: now,
    ...
    if (snapshot) {
      return {
        ...
        diagnostics: {
          ...(snapshot.diagnostics || {}),
          cacheHit: true,
          freshnessMs: now - (snapshot.lastSuccessAt || snapshot.fetchedAt || 0),
...
export async function resolveFeedPayload(feed: Feed, options?: { force?: boolean }): Promise<AggregatedFeedPayload> {
