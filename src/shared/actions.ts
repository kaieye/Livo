...
export type ActionEffectType =
  | "block"       // Don't add the entry
  | "star"        // Auto-star
  | "mark_read"   // Auto mark as read
  | "notify"      // Show desktop notification
  | "readability" // Auto-enable readability
  | "summarize"   // Auto-generate AI summary
...
export function matchCondition(
  ...
  switch (condition.field) {
    case "entry.title": fieldValue = entry.title; break
    case "entry.content": fieldValue = entry.content || ""; break
    case "entry.author": fieldValue = entry.author || ""; break
    case "entry.url": fieldValue = entry.url; break
    case "feed.title": fieldValue = feed.title; break
    case "feed.url": fieldValue = feed.url; break
    case "feed.category": fieldValue = feed.category || ""; break
  }
