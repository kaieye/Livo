import { classifyExternalUrl } from './url-policy'
...
export function isAllowedStoredMediaUrl(rawUrl: string): boolean {
  const result = classifyExternalUrl(rawUrl)
  if (!result.allowed) return false
  return !isPrivateOrLoopbackHost(result.hostname)
}

export function isAllowedStoredMediaSrcset(rawSrcset: string): boolean {
  return rawSrcset
    .split(',')
    .map((part) => part.trim().split(/\s+/)[0] || '')
    .filter(Boolean)
    .every((url) => isAllowedStoredMediaUrl(url))
}
