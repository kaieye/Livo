...
function getEditableFromTarget(
  ...
  if (
    editableNode instanceof HTMLInputElement ||
    editableNode instanceof HTMLTextAreaElement
  ) {
    return editableNode
  }
...
export function TextContextMenu() {
  ...
  useEffect(() => {
    ...
    const x =
      state.x + rect.width > window.innerWidth
        ? Math.max(6, window.innerWidth - rect.width - 6)
        : state.x
    const y =
      state.y + rect.height > window.innerHeight
        ? Math.max(6, window.innerHeight - rect.height - 6)
        : state.y
    setPos({ x, y })
      ...
      const targetEl =
        e.target instanceof HTMLElement
          ...
          : e.target instanceof Node
            ? (e.target.parentElement as HTMLElement | null)
            : null
