...
const ALLOWED_TAGS = new Set([
  ...
  "span", "small", "sub", "sup", "abbr", "cite", "q", "time",
...
const ALLOWED_ATTRS: Record<string, Set<string>> = {
  ...
  video: new Set(["src", "poster", "controls", "width", "height", "preload", "muted", "loop"]),
  audio: new Set(["src", "controls", "preload", "loop"]),
  source: new Set(["src", "type", "srcset", "media"]),
  iframe: new Set(["src", "width", "height", "allowfullscreen", "allow", "sandbox"]),
...
const GLOBAL_ATTRS = new Set(["class", "id", "style", "lang", "dir", "title", "role"])

/** Blocked URL protocols */
const BLOCKED_PROTOCOLS = /^(javascript|vbscript|data(?!:image\/))/i

/** Dangerous CSS properties */
const DANGEROUS_CSS = /expression\s*\(|url\s*\(\s*["']?javascript:|behavior\s*:|binding\s*:/i
  ...
  for (let i = 0; i < node.childNodes.length; i++) {
    ...
    if (child.nodeType === Node.ELEMENT_NODE) {
      ...
      if (!ALLOWED_TAGS.has(tagName)) {
        ...
        if (tagName === "script" || tagName === "style" || tagName === "noscript" || tagName === "object" || tagName === "embed" || tagName === "applet" || tagName === "form") {
          toRemove.push(child)
          continue
        }
