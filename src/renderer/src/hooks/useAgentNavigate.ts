...
export function handleAgentNavigationAction(
  ...
  switch (action.type) {
    ...
    case 'open-video-player':
      if (action.videoUrl) {
        void openExternalUrlSafe(action.videoUrl)
      }
      return
    case 'open-image-viewer':
      if (action.imageUrl) {
        void openExternalUrlSafe(action.imageUrl)
      }
      return
  }
}
...
export function useAgentNavigate() {
  ...
  useEffect(() => {
    const cleanup = window.api.agent.onNavigate(
      (action: AgentNavigationAction) => {
        handleAgentNavigationAction(action, navigate)
      },
    )
    return () => {
      cleanup()
