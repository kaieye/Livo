...
export function usePictureMasonry({
  ...
  useEffect(() => {
    if (!enabled) return
    const cardsToProbe = visibleCards.filter(
  ...
  const firstScreenCards = useMemo(
    () =>
      visibleCards.slice(
        0,
        Math.max(columnCount * 3, MASONRY_FIRST_SCREEN_COUNT),
      ),
  ...
  useEffect(() => {
    ...
    const ready = firstScreenCards.every(
      (card) =>
        (card.width && card.height) ||
        rememberedMasonrySizeByUrl.has(card.firstImage),
    )
    if (ready) {
      setIsFirstScreenReady(true)
      return
