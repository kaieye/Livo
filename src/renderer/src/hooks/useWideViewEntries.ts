...
export function useWideViewEntries({
  ...
  const baseFilteredEntries = useMemo(() => {
    const filtered = selectedFeedId
      ...
      : activeView !== null
        ? entries.filter((entry) => {
            ...
            if (!showRecommended && recommendedFeedIds.has(entry.feedId)) return false
    ...
    return filtered
  }, [activeView, entries, feedById, recommendedFeedIds, selectedFeedId, showRecommended])
  ...
  const renderEntries = hasStaleEntriesWhileLoading ? entries : viewFilteredEntries
    ...
    () => new Map(
      timelineEntries.map((entry) => {
        ...
        return [entry.id, {
          ...
          url: feed?.url,
        } satisfies TimelineFeedMeta] as const
      }),
    ),
    ...
    () => new Map(
      renderEntries.map((entry) => {
        ...
        return [entry.id, {
          title: feed?.title,
          imageUrl: feed?.imageUrl,
        } satisfies VideoFeedMeta] as const
      }),
    ),
