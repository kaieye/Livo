...
const timelineOffsetCache = new LRUCache<string, number>(
  TIMELINE_CACHE_CAPACITY,
)
const timelineMeasurementsCache = new LRUCache<string, unknown[]>(
  TIMELINE_CACHE_CAPACITY,
)
...
export function useTimelineView({
  ...
  const handleScroll = useCallback(
    (e: UIEvent<HTMLDivElement>) => {
      ...
      setRenderLimit((prev) =>
        Math.min(prev + SOCIAL_RENDER_BATCH, entries.length),
      )
    },
  ...
  const virtualizer = useVirtualizer({
    ...
    initialOffset: shouldUseVirtualTimeline
      ? (timelineOffsetCache.get(cacheKey) ?? 0)
      : 0,
    initialMeasurementsCache: shouldUseVirtualTimeline
      ? ((timelineMeasurementsCache.get(cacheKey) as never[] | undefined) ?? [])
      : [],
    onChange: (instance) => {
      if (!shouldUseVirtualTimeline || instance.isScrolling) return
      timelineOffsetCache.set(cacheKey, instance.scrollOffset ?? 0)
      const measurements = (instance as { measurementsCache?: unknown[] })
        .measurementsCache
