...
const PLATFORM_TITLE_SUFFIX: Record<
  'X' | 'instagram' | 'bilibili' | 'youtube',
  string
> = {
  X: 'X',
  instagram: 'Instagram',
...
const VIEW_RECOMMENDED_MAP: Record<FeedViewType, RecommendedFeed[]> = {
  ...
  [FeedViewType.Videos]:
    RECOMMENDED_VIDEO_FEEDS as unknown as RecommendedFeed[],
...
function getRecommendedUrls(rsshubInstance: string): Set<string> {
  ...
  for (const recFeeds of Object.values(VIEW_RECOMMENDED_MAP)) {
    for (const feed of recFeeds) {
      const url = feed.isRSSHub ? `${rsshubInstance}${feed.url}` : feed.url
      urls.add(url)
    }
  }
  return urls
...
function normalizePlatformFeedTitle(url: string, rawTitle: string): string {
  ...
  const withPlatform = (
    platform: 'X' | 'instagram' | 'bilibili' | 'youtube',
    user: string,
  ): string =>
    `${user.replace(/^@/, '').trim()} - ${PLATFORM_TITLE_SUFFIX[platform]}`
  const withPlatformTitle = (
    platform: 'X' | 'instagram' | 'bilibili' | 'youtube',
    fallbackUser: string,
  ): string => {
    const suffix = PLATFORM_TITLE_SUFFIX[platform]
    const cleaned = title
      .replace(new RegExp(`\\s*-\\s*${suffix}\\s*$`, 'i'), '')
      .trim()
  ...
  if (igRoute?.[1])
    return withPlatform('instagram', decodeURIComponent(igRoute[1]))
  if (host === 'www.instagram.com' || host === 'instagram.com') {
    const user = pathname.split('/').filter(Boolean)[0]
    if (user) return withPlatform('instagram', decodeURIComponent(user))
  }

  const biliRoute = pathname.match(
    /\/bilibili\/user\/(?:video|dynamic|article)\/([^/?#]+)/i,
  )
  if (biliRoute?.[1])
    return withPlatform('bilibili', decodeURIComponent(biliRoute[1]))
  ...
  if (ytRoute?.[1])
    return withPlatform('youtube', decodeURIComponent(ytRoute[1]))
...
export function useInitRecommendedFeeds(): void {
  ...
  useEffect(() => {
    ...
    const normalizedRsshubInstance = (
      rsshubInstance?.trim() || DEFAULT_RSSHUB_INSTANCE
    ).replace(/\/+$/, '')
    const syncRecommendedTitles = async (): Promise<boolean> => {
      ...
      for (const feed of currentFeeds) {
        ...
        const desiredTitle =
          titleMap.get(feed.url) ||
          normalizePlatformFeedTitle(feed.url, feed.title)
        ...
        for (const feed of recFeeds) {
          ...
          if (!existingUrls.has(url)) {
            toAdd.push({
              ...
              title:
                titleMap.get(url) ||
                normalizePlatformFeedTitle(url, feed.title),
              view,
            })
          }
        }
