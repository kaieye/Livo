...
export function normalizeExternalUrl(rawUrl: string): string {
  const decoded = trimTrailingUrlPunctuation(
    decodeHtmlEntities((rawUrl || '').trim()).replace(/\\\//g, '/'),
  )
  if (!decoded) return ''
  try {
...
function isLikelyMediaUrl(url: string): boolean {
  ...
  if (!raw) return false
  if (/\.(jpg|jpeg|png|webp|gif|bmp|avif|mp4|webm|mov|m3u8)(\?|$)/i.test(raw))
    return true
  return (
...
export function canonicalizeSocialUrl(rawUrl: string): string {
  ...
  const rssUser =
    raw.match(/^rsshub:\/\/twitter\/user\/([^/?#]+)/i) ||
    raw.match(/\/twitter\/user\/([^/?#]+)/i)
  if (rssUser?.[1]) {
