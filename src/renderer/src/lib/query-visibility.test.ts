...
describe("query-visibility", () => {
  it("invalidates queries only after the hidden threshold has elapsed", () => {
    ...
    expect(
      shouldInvalidateQueriesAfterRestore(now - QUERY_VISIBILITY_INVALIDATE_AFTER_MS + 1, now),
    ).toBe(false)
    expect(
      shouldInvalidateQueriesAfterRestore(now - QUERY_VISIBILITY_INVALIDATE_AFTER_MS, now),
    ).toBe(true)
  })
})
