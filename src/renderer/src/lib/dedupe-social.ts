...
function titlesLikelySame(a: string, b: string): boolean {
  ...
  if (
    rawA.length >= 4 &&
    rawB.length >= 4 &&
    (rawA.startsWith(rawB) || rawB.startsWith(rawA))
  )
    return true
  return false
}
...
function normalizeUrl(url: string): string {
  ...
  try {
    ...
    const xHost =
      host === 'x.com' || host === 'twitter.com' || host.includes('nitter')
...
function getEntryLeadMediaKey(entry: Entry): string {
  ...
  const fromContent =
    (entry.content || entry.summary || '').match(/https?:\/\/\S+/g) || []
...
function isPixnoySingle(entry: Entry): boolean {
  ...
  if (blob.includes('picnob.info/post/') || blob.includes('picnob.com/post/'))
    return true
  return false
}
...
function hasInstagramLikeSignal(entry: Entry): boolean {
  ...
  return urls.some((u) =>
    /instagram\.com|picnob|pixnoy|cdninstagram|fbcdn\.net/i.test(u),
  )
}
...
function getMediaRenderWeight(url: string): number {
  ...
  if (
    ...
    raw.includes('media.picnob.com/get') ||
    raw.includes('media.piokok.com/get')
  )
    return 0
...
function mergeEntries(existing: Entry, incoming: Entry): Entry {
  ...
  const mediaKeySet = new Set(
    mergedMedia.map(
      (m) =>
        `${normalizeMediaUrl(m.url || '')}|${normalizeMediaUrl(m.previewUrl || '')}|${m.type || ''}`,
    ),
  )
...
function dedupeKey(entry: Entry): string {
  ...
  if (hasInstagramLikeSignal(entry)) {
    ...
    const postIdFromContent = extractInstagramPostIdFromText(
...
function hasSocialMediaSignal(entry: Entry): boolean {
  ...
  if (
    /https?:\/\/[^\s"'<>]+\.(?:jpg|jpeg|png|webp|gif|bmp|avif)(?:\?[^\s"'<>]*)?/i.test(
      content,
    )
  )
    return true
  for (const media of entry.media || []) {
    ...
    if (url.includes('pbs.twimg.com') || preview.includes('pbs.twimg.com'))
      return true
  }
  return false
}
...
function dedupeSocialEntriesFast(entries: Entry[]): Entry[] {
  ...
  const firstPass = Array.from(byKey.values()).sort(
...
export function dedupeSocialEntries(entries: Entry[]): Entry[] {
  ...
  for (const entry of secondPass) {
    ...
    const matchedRep =
      mediaKeys
        .map((k) => mediaKeyToRep.get(`${entry.feedId}:${k}`))
        .find(Boolean) || null
    if (!matchedRep) {
      reps.push(entry)
      for (const key of mediaKeys)
        mediaKeyToRep.set(`${entry.feedId}:${key}`, entry)
      continue
    }
    ...
    for (const key of mediaKeys)
      mediaKeyToRep.set(`${entry.feedId}:${key}`, winner)
    for (const key of getEntryMediaKeys(winner))
      mediaKeyToRep.set(`${winner.feedId}:${key}`, winner)
  }
  ...
  for (const entry of thirdPass) {
    const idx = final.findIndex((existing) => {
      ...
      if (
        isGenericUntitledTitle(existing.title || '') &&
        isGenericUntitledTitle(entry.title || '')
      )
        return true
      return false
    })
  ...
  for (const entry of finalSorted) {
    const idx = titleCollapsed.findIndex((existing) => {
      ...
      const titleMatch = titlesLikelySame(
        existing.title || '',
        entry.title || '',
      )
      ...
      const contentMatch =
        !!aContent &&
        !!bContent &&
        (aContent.includes(bContent) || bContent.includes(aContent))
  ...
  for (const entry of fifthPass) {
    const idx = collapsed.findIndex((existing) => {
      ...
      return (
        Math.max(aMedia, bMedia) >= Math.max(6, Math.min(aMedia, bMedia) * 2)
      )
    })
    if (idx === -1) {
