...
export function requestLayoutFocus(target: LayoutFocusTarget) {
  window.dispatchEvent(new CustomEvent<LayoutFocusTarget>(LAYOUT_FOCUS_EVENT, { detail: target }))
}

export function subscribeLayoutFocus(listener: (target: LayoutFocusTarget) => void) {
