...
import type { AppSettings, Feed, Entry, FeedWithCount, FeedViewType, AccountProvider } from "../shared/types"
import type { ResolvedProfileUrlResult } from "../shared/types"

const api = {
  // Feed operations
  feeds: {
    add: (url: string, category?: string, view?: FeedViewType, title?: string) => ipcRenderer.invoke(IPC.FEED_ADD, url, category, view, title),
    ...
    update: (feedId: string, updates: Partial<Feed>) => ipcRenderer.invoke(IPC.FEED_UPDATE, feedId, updates),
  ...
  entries: {
    ...
    get: (entryId: string): Promise<Entry | null> => ipcRenderer.invoke(IPC.ENTRY_GET, entryId),
    markRead: (entryId: string, isRead: boolean) => ipcRenderer.invoke(IPC.ENTRY_MARK_READ, entryId, isRead),
    markAllRead: (feedId?: string) => ipcRenderer.invoke(IPC.ENTRY_MARK_ALL_READ, feedId),
    toggleStar: (entryId: string) => ipcRenderer.invoke(IPC.ENTRY_TOGGLE_STAR, entryId),
    search: (query: string, limit?: number): Promise<Entry[]> => ipcRenderer.invoke(IPC.ENTRY_SEARCH, query, limit),
  },

  // AI operations
  ai: {
    summarize: (content: string, language?: string) => ipcRenderer.invoke(IPC.AI_SUMMARIZE, content, language),
    translate: (content: string, targetLanguage: string) =>
      ipcRenderer.invoke(IPC.AI_TRANSLATE, content, targetLanguage),
    chat: (messages: Array<{ role: string; content: string }>) => ipcRenderer.invoke(IPC.AI_CHAT, messages),
    chatStream: (messages: Array<{ role: string; content: string }>, requestId: string) =>
      ipcRenderer.invoke(IPC.AI_CHAT_STREAM, messages, requestId),
    onStreamChunk: (callback: (data: { requestId: string; content: string }) => void) => {
      const handler = (_event: unknown, data: { requestId: string; content: string }) => callback(data)
    ...
    onStreamError: (callback: (data: { requestId: string; error: string }) => void) => {
      const handler = (_event: unknown, data: { requestId: string; error: string }) => callback(data)
  ...
  discover: {
    ...
    search: (query: string, platform?: "all" | "youtube" | "bilibili" | "x" | "instagram") => ipcRenderer.invoke("discover:search", query, platform),
    ...
    resolveProfileUrl: (url: string): Promise<ResolvedProfileUrlResult> => ipcRenderer.invoke(IPC.DISCOVER_RESOLVE_PROFILE_URL, url),
    probeTwitterUser: (username: string) => ipcRenderer.invoke("twitter:probe-user", username),
    probeYouTubeChannel: (query: string) => ipcRenderer.invoke("youtube:probe-channel", query),
    probeVideoSources: (query: string) => ipcRenderer.invoke("discover:probe-video-sources", query),
    probeBilibiliUid: (uid: string) => ipcRenderer.invoke("discover:probe-bilibili-uid", uid),
    probeBilibiliUsers: (query: string) => ipcRenderer.invoke("discover:probe-bilibili-users", query),
    probeInstagramUser: (username: string) => ipcRenderer.invoke("instagram:probe-user", username),
  },

  // App
  app: {
    getVersion: (): Promise<string> => ipcRenderer.invoke("app:version"),
    openExternal: (url: string): Promise<{ success: boolean }> => ipcRenderer.invoke(IPC.APP_OPEN_EXTERNAL, url),
  },

  // Data maintenance
  data: {
    cleanup: (options?: { entriesPerFeed?: number; maxEntryAgeDays?: number }) =>
  ...
  video: {
    resolve: (url: string): Promise<{ success: boolean; url?: string; quality?: string; title?: string; error?: string }> =>
  ...
  accounts: {
    status: (provider: AccountProvider): Promise<{ provider: AccountProvider; linked: boolean; displayName?: string | null; error?: string }> =>
      ipcRenderer.invoke(IPC.ACCOUNT_STATUS, provider),
    link: (provider: AccountProvider): Promise<{ success: boolean; error?: string }> =>
      ipcRenderer.invoke(IPC.ACCOUNT_LINK, provider),
    unlink: (provider: AccountProvider): Promise<{ success: boolean; error?: string }> =>
      ipcRenderer.invoke(IPC.ACCOUNT_UNLINK, provider),
    setDisplayName: (provider: AccountProvider, displayName: string): Promise<{ success: boolean; error?: string }> =>
